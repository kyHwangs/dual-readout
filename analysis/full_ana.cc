...
float GetDepthScint(float time) {
  return (21.6544 - time) / 2.9284;
}

float GetDepthCeren(float time) {
  return (19.2205 - time) / 1.9549;
}
...
int main(int argc, char* argv[]) {
  ...
  while (drInterface->numEvt() < entries) {
    ...
    tTmax_C->Fill(tMaxTimeCeren);
    tTmax_S->Fill(tMaxTimeScint);

    float tMaxDepthCeren = GetDepthCeren(tMaxTimeCeren);
    float tMaxDepthScint = GetDepthScint(tMaxTimeScint);
