...
int main(int argc, char* argv[]) {
  ...
  TH1F* tNpe_C = new TH1F("Npe_C","Number of Cerenkov p.e.;p.e.;Evt",200,0.,3500.);
  tNpe_C->Sumw2(); tNpe_C->SetLineColor(kBlue); tNpe_C->SetLineWidth(2);
  TH1F* tNpe_S = new TH1F("Npe_S","Number of Scint p.e.;p.e.;Evt",200,0.,40000.);
  tNpe_S->Sumw2(); tNpe_S->SetLineColor(kRed); tNpe_S->SetLineWidth(2);
  ...
  float fCalibCeren = 20. / 1336.;
  float fCalibScint = 20. / 21920.;

  std::vector<float> E_Cs,E_Ss;
  ...
  while (drInterface->numEvt() < entries) {
      ...
      for (auto sipm = tower->SiPMs.begin(); sipm != tower->SiPMs.end(); ++sipm) {
        if ( RecoInterface::IsCerenkov(sipm->x,sipm->y) ) {
          ...
          for (const auto timepair : sipm->timeStruct) {
            ...
            if (timepair.first.second < 30.) nCHit += timepair.second;
          }
          tNhit_C->Fill(nCHit);
          nC += nCHit;
          ...
          for (const auto timepair : sipm->timeStruct) {
            ...
            if (timepair.first.second < 70.) nSHit += timepair.second;
          }
          tNhit_S->Fill(nSHit);
          nS += nSHit;
          for (const auto wavpair : sipm->wavlenSpectrum) {
    ...
    float E_S = (float)nS*fCalibScint;
    float E_C = (float)nC*fCalibCeren;
  ...
  tP_leak->Draw("Hist"); 
  c->SaveAs(filename+"_Pleak.png");
  
  tP_leak_nu->Draw("Hist"); 
  c->SaveAs(filename+"_Pleak_nu.png");
  ...
  tNpe_C->Draw("Hist"); c->SaveAs(filename+"_NpeC.png");
  tNpe_S->Draw("Hist"); c->SaveAs(filename+"_NpeS.png");
}
